...
var cryptoKey = "Crowcial만 알 수 있는 비밀 키"; // 이 라우터 모듈은 접속한 사용자가 desktop 유저인지 phone 유저인지를 비교하는 모듈을 사용함.
...
var usersearchpug = fs.readFileSync(path.join(__dirname, "../views/usersearch.pug"), 'utf8');
var newpasswordpug = fs.readFileSync(path.join(__dirname, "../views/usersearch-newpassword.pug"), 'utf8');
var newpasswordCompletepug = fs.readFileSync(path.join(__dirname, "../views/usersearch-newpassword-complete.pug"), 'utf8'); // GET 접속하면 화면을 보여줌.
...
router.get('/mailsend', function (req, res) {
...
router.get('/mailcert', function (req, res) {
  if (req.device.type == 'desktop') {
...
router.post('/pw/new', function (req, res) {
  // 데스크탑
  if (req.device.type == 'desktop') {
