...
var device = require('express-device'); // 이 라우터 모듈은 접속한 사용자가 desktop 유저인지 phone 유저인지를 비교하는 모듈을 사용함.
...
var userpug = fs.readFileSync(path.join(__dirname, "../views/user.pug"), 'utf8');
var userCompletepug = fs.readFileSync(path.join(__dirname, "../views/usercompleteproject.pug"), 'utf8');
var suppedpug = fs.readFileSync(path.join(__dirname, "../views/supped.pug"), 'utf8');
...
router.post('/withdraw', function (req, res) {
  if (req.device.type == 'desktop') {
    //컴퓨터 인터넷
    connection.query('UPDATE USER SET USER_EXIT=? WHERE USER_ID=?', [1, req.session.login_id], function () {
      // 회원탈퇴신청 완료 화면을 보여줌.
  ...
  } else if (req.device.type == 'phone') //핸드폰 경우
    {
      ...
      connection.query(query, params, function (err, session1) {
        ...
        if (session1.length > 0) {
          ...
          connection.query(query2, params2, function (err, session2) {
            // 회원탈퇴신청 완료 화면을 보여줌.
  ...
  connection.query(query, params, function (err, rows) {
    if (!rows.length > 0) {
      ...
      connection.query(query, params, function (err, rows) {
        ...
        var params = [projectnum]; // 해당 프로젝트의 좋아요
...
router.post('/:id/projectdel', function (req, res) {
  projectnum = req.params.id;
  id = req.session.login_usernum;
  console.log("PROJECT DELETE" + projectnum); // 외래키 제약 조건 때문에 지워지지 않아서 설정
