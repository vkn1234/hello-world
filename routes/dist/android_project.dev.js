...
router.post('/donation/retrieving', function (req, res) {
  ...
  if (req.device.type == "phone") {
    ...
    connection.query(query, params, function (err, results) {
      if (err) console.log(err); // 만약 이미 모급 환급이 된 프로젝트이면 금액을 환불하면 안되니 프로젝트 정보를 조회

      var query = 'SELECT * FROM PROJECT WHERE PROJECT_NUM=?';
      var params = [req.body.projectnum];
      connection.query(query, params, function (err, rows) {
        if (err) console.log(err); // 아직 모금 환급이 되지 않은 상태이면 후원금 환불이 가능함
...
router.get('/donation/lookup', function (req, res) {
  if (req.device.type == "phone") {
    // 사용자가 후원한 프로젝트들만 조회함.
    ...
    connection.query(query, params, function (err, projects) {
      if (err) console.log(err);
      res.status(200).json({
        projects: projects
      });
  ...
  if (req.device.type == "phone") {
    ...
    connection.query(query, params, function (err, rows) {
      ...
      if (rows.length <= 0) {
        ...
        var params = [req.body.money, req.body.projectnum];
        connection.query(query, params, function (err, rows) {
          if (err) console.log(err); // 사용자의 MONEY 정보를 업데이트한다.

          var query = 'UPDATE USERMONEY SET USER_NOW=USER_NOW-?, USER_PSEND=USER_PSEND+? WHERE USER_NUM=?';
          var params = [req.body.money, req.body.money, req.body.usernum];
          connection.query(query, params, function (err, rows) {
            if (err) console.log(err); // 해당 사용자가 해당 프로젝트에 얼마만큼 후원을 했음을 기록한다.
  ...
  if (req.device.type == "phone") {
    ...
    connection.query(query, params, function (err, rows) {
      ...
      if (rows.length > 0) {
        ...
        var params = [req.body.usernum, req.body.projectnum];
        connection.query(query, params, function (err, rows) {
          if (err) console.log(err); // 해당 프로젝트의 좋아요 수를 1 증가
...
router.get('/content', function (req, res) {
  if (req.device.type == "phone") {
    ...
    connection.query(query, params, function (err, projects) {
      if (err) console.log(err); // 사용자가 좋아요를 누른 프로젝트가 무엇인지에 대한 정보를 가져옴

      var query = 'SELECT PROJECT_NUM FROM LIKEPROJECT WHERE USER_NUM=?';
      var params = [req.query.usernum];
      connection.query(query, params, function (err, likes) {
        if (err) console.log(err);
        res.status(200).json({
          projects: projects,
          likes: likes
